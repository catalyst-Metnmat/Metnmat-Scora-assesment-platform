/* SCORA Assessment Designer — reusable module.
 * Mounts natively into any container (the HR/Director dashboard's Designer tab,
 * or the standalone /admin page). All state is encapsulated; the host injects
 * auth via mount({ authHeaders, role, onError, onKeyChange, toast }). */
window.Designer = (function () {
  let app = null, FW = null, tab = 'skills', dirty = false, expandedSkill = null, ROLE = 'hr';
  let _authHeaders = () => ({}), _onError = () => {}, _onKeyChange = null, _toastFn = null;
  ...
  async function api(path, opts = {}) {
    ...
    if (res.status === 401 || res.status === 403) { _onError(); throw new Error('forbidden'); }
  ...
  function render() {
    ...
    app.innerHTML = `
      <div class="designer-bar">
        <span class="muted"><b>${FW.domains.length}</b> categories · <b>${totalSkills}</b> skills · changes apply to <b>future</b> cycles</span>
        <div class="actions">
          <button class="btn" id="saveBtn" disabled>Saved</button>
          <button class="btn secondary small" id="importBtn">Import Excel / PDF</button>
          <button class="btn ghost small" id="exportXlsxBtn">Export Excel</button>
  ...
  async function exportXlsx() {
    ...
    a.href = URL.createObjectURL(blob); a.download = 'METNMAT_assessment_data.xlsx'; a.click();
  ...
  async function onImportFile(e) {
    ...
    let j;
    try {
      const res = await fetch('/api/admin/import', { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/octet-stream', 'X-Filename': encodeURIComponent(file.name) }, body: buf });
      if (res.status === 401 || res.status === 403) { _onError(); return; }
      j = await res.json();
      if (!res.ok) throw new Error(j.error || 'Import failed');
    } catch (err) { toast(err.message); return; }

    const preview = j.draft.domains.map(d => `<li style="padding:5px 0;border-bottom:1px dashed var(--line)"><b>${esc(d.code)}</b> — ${esc(d.name)} <span class="muted">(${d.skills.length} questions)</span></li>`).join('');
    document.getElementById('importPanel').innerHTML = `
      <div class="card" style="border-left:4px solid var(--copper)">
        ...
        <p class="muted" style="margin-bottom:12px">This is a <b>draft</b> — nothing is saved yet. Load it into the editor, adjust, then click <b>Save changes</b>.</p>
        <div class="actions">
  ...
  function renderKeys(body) {
    body.innerHTML = `
      <div class="card">
        ...
        <p class="muted" style="margin-bottom:12px">Use this when HR forgets their key or it needs rotating. The new key takes effect immediately. Overrides the <code>HR_KEY</code> environment variable.</p>
        <div class="actions">
      ...
      <div class="card">
        ...
        </div><div id="adKeyResult"></div>
      </div>
      <div class="card">
        <h3>Security notes</h3>
        <ul class="rules mt">
          <li>Keys are checked with constant-time comparison and a lockout after repeated failures.</li>
          <li>Named user accounts (Director → Users) are the preferred sign-in; these shared keys are a fallback.</li>
          <li>Every key change is recorded in the audit log (the key value itself is never stored).</li>
        </ul>
      </div>`;
    ...
    async function setKey(role, inputId, resultId) {
      ...
      if (!confirm(role === 'hr' ? 'Reset the HR key now? The old HR key stops working immediately.' : 'Change YOUR admin key now? Make sure you save the new key.')) return;
      try {
        await api('/api/admin/keys', { method: 'PUT', body: JSON.stringify({ role, key }) });
        if (_onKeyChange) _onKeyChange(role, key); // let the host keep its session alive on the new key
        result.innerHTML = `
          <div class="mt" style="background:var(--copper-pale);border:1px dashed var(--copper);border-radius:10px;padding:12px 14px">
            <b>${role === 'hr' ? 'New HR key' : 'New admin key'}:</b> <code style="font-size:15px">${esc(key)}</code>
            <button class="btn ghost small" id="copyKey" style="margin-left:10px">Copy</button>
            <div class="muted" style="margin-top:6px">${role === 'hr' ? 'Share this with HR securely. The old key no longer works.' : 'Saved — your session has switched to it.'}</div>
          </div>`;
      ...
      } catch (e) { if (e.message !== 'forbidden') result.innerHTML = `<div class="error-msg">${esc(e.message)}</div>`; }
    }
  ...
  function renderSkills(body) {
    ...
    body.querySelectorAll('[data-adv]').forEach(b => b.onclick = () => { expandedSkill = expandedSkill === b.dataset.adv ? null : b.dataset.adv; renderTab(); });
    ...
    body.querySelectorAll('.admin-skill input').forEach(el => el.addEventListener('input', () => { FW.domains[+el.dataset.d].skills[+el.dataset.s].name = el.value; markDirty(); }));
      ...
      <div class="card">
        ...
        ${FW.scale.map((s, i) => `
          <div class="admin-scale">
            ...
            <input data-sc="${i}" data-k="label" value="${esc(s.label)}" placeholder="Label">
            <input data-sc="${i}" data-k="definition" value="${esc(s.definition)}" placeholder="Definition">
  ...
  async function mount(container, opts = {}) {
    app = container;
    _authHeaders = opts.authHeaders || (() => ({}));
    _onError = opts.onError || (() => {});
    _onKeyChange = opts.onKeyChange || null;
    _toastFn = opts.toast || null;
    ROLE = opts.role || 'hr';
    tab = 'skills'; dirty = false; expandedSkill = null;
    app.innerHTML = '<div class="empty loading">Loading designer…</div>';
    try { FW = await api('/api/admin/framework'); render(); }
    catch (e) { if (e.message !== 'forbidden') app.innerHTML = `<div class="empty">${esc(e.message)}</div>`; }
  }
  return { mount, isDirty: () => dirty };
})();
